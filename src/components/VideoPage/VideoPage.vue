...
interface DetailType {
  ...
  image_url: string;
  video_url: string;
  video_playNumber: number;
  video_commentNumber: number;
}
...
async function fetchData() {
  ...
  try {
    const response = await fetch('http://api.coleduck.cn/play_video/' + videoID.value + '/'); // 替换成实际的API地址
...
onMounted(() => {
  ...
  videoID.value = urlParams.get('video_id');
      ...
      <v-col cols="8">
        <v-col>
          <v-row class="text-black text-h5 mb-1">
            <span class="ml-2"> {{details.video_title}} </span>
          </v-row>
          <v-row class="text-grey mb-1">
            <v-icon class="ml-2" icon="mdi-play-outline"></v-icon>
            <span class="ml-1">{{details.video_playNumber}}</span>
            <v-icon class="ml-2" icon="mdi-comment-text-outline"></v-icon>
            <span class="ml-1">{{details.video_commentNumber}}</span>
